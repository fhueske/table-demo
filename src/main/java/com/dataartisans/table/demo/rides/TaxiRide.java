...
package com.dataartisans.table.demo.rides;
...
public class TaxiRide {
	...
	public TaxiRide(long rideId, boolean isStart, long startTime, long endTime,
	...
	public String toString() {
		...
		if (isStart) {
			sb.append(timeFormatter.print(startTime)).append(",");
			sb.append(timeFormatter.print(endTime)).append(",");
		} else {
			sb.append(timeFormatter.print(endTime)).append(",");
			sb.append(timeFormatter.print(startTime)).append(",");
		}
	...
	public static TaxiRide fromString(String line) {
		...
		try {
			...
			switch (tokens[1]) {
				case "START":
					ride.isStart = true;
					ride.startTime = timeFormatter.parseMillis(tokens[2]);
					ride.endTime = timeFormatter.parseMillis(tokens[3]);
					break;
				case "END":
					ride.isStart = false;
					ride.endTime = timeFormatter.parseMillis(tokens[2]);
					ride.startTime = timeFormatter.parseMillis(tokens[3]);
					break;
				default:
					throw new RuntimeException("Invalid record: " + line);
			}
